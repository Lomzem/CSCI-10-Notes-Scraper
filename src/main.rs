...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let modules_url = Url::parse_with_params(
        canvas_url.join(modules_endpoint.as_ref())?
            .as_str(),
        &api_params,
    )?;

    let module_list: Vec<Module> = reqwest::get(modules_url)
        .await?
        .json()
        .await?;
    ...
    let list_module_endpoint = format!(
        "GET /api/v1/courses/{}/modules/{}/items",
        CSCI_10_ID,
        notes_module.id,
    );

    let list_module_url = Url::parse_with_params(
        canvas_url.join(list_module_endpoint.as_ref())?
            .as_str(),
        &api_params,
    );

    println!("{:?}", notes_module);
    // std::fs::write("response.json", body?.as_bytes())?;
